...
#else
  #define CASADI_PREFIX(ID) FKinBody_ ## ID
#endif
...
/* FKinBody:(i0[4x4],i1[6x6],i2[6])->(o0[4x4]) */
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  casadi_int i, j, k;
  casadi_real **res1=res+1, *rr, *ss, *tt;
  const casadi_real **arg1=arg+3;
  casadi_real *w0=w+199, *w1=w+215, *w2=w+231, *w3=w+247, *w4=w+263, *w5=w+279, *w6=w+295, *w7=w+311, *w8=w+347, *w9=w+353, w10, *w11=w+360, *w12=w+376;
  /* #0: @0 = zeros(4x4) */
  casadi_clear(w0, 16);
  /* #1: @1 = zeros(4x4) */
  casadi_clear(w1, 16);
  ...
  casadi_clear(w5, 16);
  /* #6: @6 = input[0][0] */
  casadi_copy(arg[0], 16, w6);
  /* #7: @7 = input[1][0] */
  casadi_copy(arg[1], 36, w7);
  /* #8: @8 = @7[:6] */
  for (rr=w8, ss=w7+0; ss!=w7+6; ss+=1) *rr++ = *ss;
  /* #9: @9 = input[2][0] */
  casadi_copy(arg[2], 6, w9);
  /* #10: @10 = @9[0] */
  for (rr=(&w10), ss=w9+0; ss!=w9+1; ss+=1) *rr++ = *ss;
  /* #11: @8 = (@8*@10) */
  for (i=0, rr=w8; i<6; ++i) (*rr++) *= w10;
  /* #12: @11 = VecTose3(@8) */
  arg1[0]=w8;
  res1[0]=w11;
  ...
  /* #14: @5 = mac(@6,@12,@5) */
  for (i=0, rr=w5; i<4; ++i) for (j=0; j<4; ++j, ++rr) for (k=0, ss=w6+j, tt=w12+i*4; k<4; ++k) *rr += ss[k*4]**tt++;
  /* #15: @8 = @7[6:12] */
  for (rr=w8, ss=w7+6; ss!=w7+12; ss+=1) *rr++ = *ss;
  /* #16: @10 = @9[1] */
  ...
  if (casadi_f3(arg1, res1, iw, w, 0)) return 1;
  /* #20: @4 = mac(@5,@12,@4) */
  for (i=0, rr=w4; i<4; ++i) for (j=0; j<4; ++j, ++rr) for (k=0, ss=w5+j, tt=w12+i*4; k<4; ++k) *rr += ss[k*4]**tt++;
  /* #21: @8 = @7[12:18] */
  for (rr=w8, ss=w7+12; ss!=w7+18; ss+=1) *rr++ = *ss;
  /* #22: @10 = @9[2] */
  ...
  if (casadi_f3(arg1, res1, iw, w, 0)) return 1;
  /* #26: @3 = mac(@4,@12,@3) */
  for (i=0, rr=w3; i<4; ++i) for (j=0; j<4; ++j, ++rr) for (k=0, ss=w4+j, tt=w12+i*4; k<4; ++k) *rr += ss[k*4]**tt++;
  /* #27: @8 = @7[18:24] */
  for (rr=w8, ss=w7+18; ss!=w7+24; ss+=1) *rr++ = *ss;
  /* #28: @10 = @9[3] */
  ...
  if (casadi_f3(arg1, res1, iw, w, 0)) return 1;
  /* #32: @2 = mac(@3,@12,@2) */
  for (i=0, rr=w2; i<4; ++i) for (j=0; j<4; ++j, ++rr) for (k=0, ss=w3+j, tt=w12+i*4; k<4; ++k) *rr += ss[k*4]**tt++;
  /* #33: @8 = @7[24:30] */
  for (rr=w8, ss=w7+24; ss!=w7+30; ss+=1) *rr++ = *ss;
  /* #34: @10 = @9[4] */
  for (rr=(&w10), ss=w9+4; ss!=w9+5; ss+=1) *rr++ = *ss;
  /* #35: @8 = (@8*@10) */
  for (i=0, rr=w8; i<6; ++i) (*rr++) *= w10;
  /* #36: @3 = VecTose3(@8) */
  arg1[0]=w8;
  ...
  if (casadi_f3(arg1, res1, iw, w, 0)) return 1;
  /* #38: @1 = mac(@2,@12,@1) */
  for (i=0, rr=w1; i<4; ++i) for (j=0; j<4; ++j, ++rr) for (k=0, ss=w2+j, tt=w12+i*4; k<4; ++k) *rr += ss[k*4]**tt++;
  /* #39: @8 = @7[30:36] */
  for (rr=w8, ss=w7+30; ss!=w7+36; ss+=1) *rr++ = *ss;
  /* #40: @10 = @9[5] */
  ...
  if (casadi_f3(arg1, res1, iw, w, 0)) return 1;
  /* #44: @0 = mac(@1,@12,@0) */
  for (i=0, rr=w0; i<4; ++i) for (j=0; j<4; ++j, ++rr) for (k=0, ss=w1+j, tt=w12+i*4; k<4; ++k) *rr += ss[k*4]**tt++;
  /* #45: output[0][0] = @0 */
...
CASADI_SYMBOL_EXPORT int FKinBody_work_bytes(casadi_int *sz_arg, casadi_int* sz_res, casadi_int *sz_iw, casadi_int *sz_w) {
  ...
  if (sz_w) *sz_w = 392*sizeof(casadi_real);
  return 0;
}

#ifdef MATLAB_MEX_FILE
void mex_FKinBody(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  int mem;
  casadi_real w[466];
  casadi_int *iw = 0;
  ...
  casadi_real* res[7] = {0};
  if (argc>3) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"FKinBody\" failed. Too many input arguments (%d, max 3)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"FKinBody\" failed. Too many output arguments (%d, max 1)", resc);
  ...
  res[0] = w+58;
  FKinBody_incref();
  mem = FKinBody_checkout();
  i = FKinBody(arg, res, iw, w+74, mem);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"FKinBody\" failed.");
  FKinBody_release(mem);
  FKinBody_decref();
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_FKinBody(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "FKinBody")==0) {
    mex_FKinBody(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'FKinBody'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
