...
#else
  #define CASADI_PREFIX(ID) so3ToVec_ ## ID
#endif
...
#ifdef MATLAB_MEX_FILE
void mex_so3ToVec(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  int mem;
  casadi_real w[22];
  casadi_int *iw = 0;
  ...
  casadi_real* res[2] = {0};
  if (argc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"so3ToVec\" failed. Too many input arguments (%d, max 1)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"so3ToVec\" failed. Too many output arguments (%d, max 1)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+12);
  --resc;
  res[0] = w+9;
  so3ToVec_incref();
  mem = so3ToVec_checkout();
  i = so3ToVec(arg, res, iw, w+12, mem);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"so3ToVec\" failed.");
  so3ToVec_release(mem);
  so3ToVec_decref();
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_so3ToVec(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "so3ToVec")==0) {
    mex_so3ToVec(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'so3ToVec'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
