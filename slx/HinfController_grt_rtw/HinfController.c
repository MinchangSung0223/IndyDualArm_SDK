...
void HinfController_step(void)
{
  ...
  for (i = 0; i <= 10; i += 2) {
    ...
    _mm_storeu_pd(&rtb_eint[i], _mm_add_pd(_mm_mul_pd(tmp_1, _mm_set1_pd
      (HinfController_U.dt)), _mm_loadu_pd(&HinfController_U.eint[i])));
  }

  memset(&a[0], 0, 144U * sizeof(real_T));
  for (i = 0; i < 12; i++) {
    a[i + 12 * i] = HinfController_U.HinfK[i];
    _mm_storeu_pd(&tmp_2[0], _mm_add_pd(_mm_add_pd(_mm_mul_pd(_mm_set_pd(20.0,
  ...
  for (i = 0; i < 12; i++) {
    ...
    for (i_0 = 0; i_0 < 12; i_0++) {
      tmp_3 = 12 * i_0 + i;
      _mm_storeu_pd(&tmp_2[0], _mm_add_pd(_mm_mul_pd(_mm_set_pd(a[tmp_3],
        HinfController_U.M[tmp_3]), _mm_set_pd(edot_0[i_0], tmp[i_0])),
    ...
    /* Outport: '<Root>/eint' */
    HinfController_Y.eint[i] = rtb_eint[i];
  }
