...
#else
  #define CASADI_PREFIX(ID) VecToso3_ ## ID
#endif
...
#ifdef MATLAB_MEX_FILE
void mex_VecToso3(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  int mem;
  casadi_real w[42];
  casadi_int *iw = 0;
  ...
  casadi_real* res[2] = {0};
  if (argc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"VecToso3\" failed. Too many input arguments (%d, max 1)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"VecToso3\" failed. Too many output arguments (%d, max 1)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+12);
  --resc;
  res[0] = w+3;
  VecToso3_incref();
  mem = VecToso3_checkout();
  i = VecToso3(arg, res, iw, w+12, mem);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"VecToso3\" failed.");
  VecToso3_release(mem);
  VecToso3_decref();
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_VecToso3(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "VecToso3")==0) {
    mex_VecToso3(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'VecToso3'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
