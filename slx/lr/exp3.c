...
#else
  #define CASADI_PREFIX(ID) exp3_ ## ID
#endif
...
#ifdef MATLAB_MEX_FILE
void mex_exp3(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  int mem;
  casadi_real w[94];
  casadi_int *iw = 0;
  ...
  casadi_real* res[3] = {0};
  if (argc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"exp3\" failed. Too many input arguments (%d, max 1)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"exp3\" failed. Too many output arguments (%d, max 1)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+12);
  --resc;
  res[0] = w+3;
  exp3_incref();
  mem = exp3_checkout();
  i = exp3(arg, res, iw, w+12, mem);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"exp3\" failed.");
  exp3_release(mem);
  exp3_decref();
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_exp3(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "exp3")==0) {
    mex_exp3(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'exp3'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
