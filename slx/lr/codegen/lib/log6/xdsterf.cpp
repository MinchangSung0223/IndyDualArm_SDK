...
int xdsterf(double d[2], double e)
{
  ...
  do {
    ...
    if (l1 > 2) {
      ...
      exitg2 = false;
      while ((!exitg2) && (m < 2)) {
        if (std::abs(e) <= std::sqrt(std::abs(d[0])) *
                               std::sqrt(std::abs(d[1])) *
      ...
      if (m != l) {
        ...
        if (!(anorm == 0.0)) {
          ...
          if (anorm > 2.2346346549904327E+153) {
            ...
            xzlascl(anorm, 2.2346346549904327E+153, n_tmp, e);
          } else if (anorm < 3.02546243347603E-123) {
            iscale = 2;
            xzlascl(anorm, 3.02546243347603E-123, n_tmp + 1, d, l);
            xzlascl(anorm, 3.02546243347603E-123, n_tmp, e);
          }
          for (i = l; i < m; i++) {
            e *= e;
          }
          ...
          if (lend >= l) {
            ...
            do {
              ...
              if (l != lend) {
                m = l;
                while ((m < lend) && (!(std::abs(e) <= 4.9303806576313238E-32 *
                                                           std::abs(d[0]) *
                                                           std::abs(d[1])))) {
                  m = 2;
                }
              ...
              if (m == l) {
                ...
                d[0] = xdlaev2(d[0], std::sqrt(e), d[1], d[1]);
                e = 0.0;
                exitg4 = 1;
              } else if (jtot == 60) {
                exitg4 = 1;
                ...
                s = d[l - 1];
                sigma = (d[1] - s) / (2.0 * rte);
                b_anorm = std::abs(sigma);
                if (b_anorm < 1.0) {
                  b_anorm = std::sqrt(b_anorm * b_anorm + 1.0);
                } else if (b_anorm > 1.0) {
                  p = 1.0 / b_anorm;
                  b_anorm *= std::sqrt(p * p + 1.0);
                } else {
            ...
            do {
              ...
              m = l;
              while ((m > 1) &&
                     (!(std::abs(e) <= 4.9303806576313238E-32 * std::abs(d[1]) *
                                           std::abs(d[0])))) {
                m = 1;
              }
              if (m > 1) {
              ...
              if (m == l) {
                ...
                d[l - 1] = xdlaev2(d[l - 1], std::sqrt(e), d[0], rte);
                d[0] = rte;
                e = 0.0;
                exitg3 = 1;
              } else if (jtot == 60) {
          ...
          if (iscale == 1) {
            xzlascl(2.2346346549904327E+153, anorm, lendsv_tmp_tmp - lsv, d,
                    lsv);
          } else if (iscale == 2) {
            xzlascl(3.02546243347603E-123, anorm, lendsv_tmp_tmp - lsv, d, lsv);
          }
          if (jtot >= 60) {
            if (e != 0.0) {
              info = 1;
