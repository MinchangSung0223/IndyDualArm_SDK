...
Robot::Robot( class b3RobotSimulatorClientAPI* sim, int robot_id,JVec q0){
    // Initialize the robot with GUI
    this->sim=sim;
    this->robot_id=robot_id;
    this->InitializeRobot();
...
void Robot::set_q(JVec q ){
	b3RobotSimulatorJointMotorArgs controlArgs(CONTROL_MODE_POSITION_VELOCITY_PD);
	for (int i = 0; i<q.size();i++){
		controlArgs.m_maxTorqueValue = 10000;
		controlArgs.m_kd = 0.02;
		controlArgs.m_kp = 1.0;		
		controlArgs.m_targetPosition  =q[i];
		sim->setJointMotorControl(this->robot_id,this->actuated_joint_id.at(i),controlArgs);
	}	
}
