...
#else
  #define CASADI_PREFIX(ID) dexp3_ ## ID
#endif
...
#ifdef MATLAB_MEX_FILE
void mex_dexp3(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  int mem;
  casadi_real w[85];
  casadi_int *iw = 0;
  ...
  casadi_real* res[3] = {0};
  if (argc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"dexp3\" failed. Too many input arguments (%d, max 1)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"dexp3\" failed. Too many output arguments (%d, max 1)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+12);
  --resc;
  res[0] = w+3;
  dexp3_incref();
  mem = dexp3_checkout();
  i = dexp3(arg, res, iw, w+12, mem);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"dexp3\" failed.");
  dexp3_release(mem);
  dexp3_decref();
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_dexp3(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "dexp3")==0) {
    mex_dexp3(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'dexp3'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
