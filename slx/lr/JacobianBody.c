...
#else
  #define CASADI_PREFIX(ID) JacobianBody_ ## ID
#endif
...
/* JacobianBody:(i0[6x6],i1[6])->(o0[6x6]) */
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  casadi_int i, j, k;
  casadi_real **res1=res+1, *rr, *ss, *tt;
  const casadi_real **arg1=arg+2, *cs;
  casadi_real *w0=w+202, *w1=w+238, *w2=w+244, *w3=w+280, *w4=w+286, w5, *w6=w+293, *w7=w+309, *w8=w+325, *w9=w+361, *w10=w+377;
  /* #0: @0 = zeros(6x6) */
  casadi_clear(w0, 36);
  /* #1: @1 = zeros(6x1) */
  casadi_clear(w1, 6);
...
#ifdef MATLAB_MEX_FILE
void mex_JacobianBody(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  int mem;
  casadi_real w[471];
  casadi_int *iw = 0;
  ...
  casadi_real* res[7] = {0};
  if (argc>2) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"JacobianBody\" failed. Too many input arguments (%d, max 2)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"JacobianBody\" failed. Too many output arguments (%d, max 1)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+78);
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+36, casadi_s1, w+78);
  --resc;
  res[0] = w+42;
  JacobianBody_incref();
  mem = JacobianBody_checkout();
  i = JacobianBody(arg, res, iw, w+78, mem);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"JacobianBody\" failed.");
  JacobianBody_release(mem);
  JacobianBody_decref();
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_JacobianBody(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "JacobianBody")==0) {
    mex_JacobianBody(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'JacobianBody'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
