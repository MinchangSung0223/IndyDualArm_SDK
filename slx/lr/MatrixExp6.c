...
#else
  #define CASADI_PREFIX(ID) MatrixExp6_ ## ID
#endif
...
CASADI_SYMBOL_EXPORT int MatrixExp6_work_bytes(casadi_int *sz_arg, casadi_int* sz_res, casadi_int *sz_iw, casadi_int *sz_w) {
  ...
  if (sz_w) *sz_w = 199*sizeof(casadi_real);
  return 0;
}

#ifdef MATLAB_MEX_FILE
void mex_MatrixExp6(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  int mem;
  casadi_real w[231];
  casadi_int *iw = 0;
  ...
  casadi_real* res[6] = {0};
  if (argc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"MatrixExp6\" failed. Too many input arguments (%d, max 1)", argc);
  if (resc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"MatrixExp6\" failed. Too many output arguments (%d, max 1)", resc);
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+32);
  --resc;
  res[0] = w+16;
  MatrixExp6_incref();
  mem = MatrixExp6_checkout();
  i = MatrixExp6(arg, res, iw, w+32, mem);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"MatrixExp6\" failed.");
  MatrixExp6_release(mem);
  MatrixExp6_decref();
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_MatrixExp6(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "MatrixExp6")==0) {
    mex_MatrixExp6(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'MatrixExp6'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
