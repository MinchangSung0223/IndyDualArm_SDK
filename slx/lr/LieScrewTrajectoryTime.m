function [T,V,Vdot]=LieScrewTrajectoryTime(X_start,X_end,V_start,V_end,Vdot_start,Vdot_end,Tf,t)

    if t>Tf
        t = Tf;
    end
    lambda0 = zeros(6,1);
    lambdaT = se3ToVec(MatrixLog6(TransInv(X_start)*X_end));
    
    lambda_dot0 = V_start;
    lambda_ddot0 =Vdot_start;
    lambda_dotT = dlog6(-lambdaT)*V_end;
    lambda_ddotT =dlog6(-lambdaT)*Vdot_end+ddlog6(-lambdaT,-lambda_dotT)*V_end;
    
    lambda_t = zeros(6,1);
    lambda_dot_t = zeros(6,1);
    lambda_ddot_t = zeros(6,1);
    for j =1:1:6
    ...
    V = dexp6(-lambda_t)*lambda_dot_t;
    Vdot = dexp6(-lambda_t)*lambda_ddot_t+ ddexp6(-lambda_t,-lambda_dot_t)*lambda_dot_t;
    T = X_start*MatrixExp6(VecTose3(lambda_t));
end
